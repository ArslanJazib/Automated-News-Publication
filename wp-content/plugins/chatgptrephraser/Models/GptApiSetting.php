...
class GptApiSetting {
    ...
    public function create_record($api_url = null, $api_key = null, $model_id = null) {
        global $wpdb;
    ...
    public function get_api_settings() {
        global $wpdb;
        $query = "SELECT api_url, api_key, model_id FROM $this->table_name LIMIT 1";
    ...
    public function get_model_id($settingsId) {
        global $wpdb;
        $query = $wpdb->prepare(
            "SELECT model_id FROM $this->table_name WHERE id = %d",
            $settingsId
        );
        $result = $wpdb->get_var($query);
    ...
    public function get_model_name($model_id) {
        global $wpdb;
        $model_settings_table = $wpdb->prefix . 'gpt_api_model_settings';
        $query = $wpdb->prepare(
            "SELECT m.model_name
            FROM $model_settings_table m
            JOIN $this->table_name a ON m.id = a.model_id
            WHERE a.model_id = %d",
            $model_id
        );
    ...
    public function get_model_name_by_id($model_id) {
        global $wpdb;
        $model_settings_table = $wpdb->prefix . 'gpt_api_model_settings';
        $query = $wpdb->prepare(
            "SELECT model_name
            FROM $model_settings_table 
            WHERE id = %d",
            $model_id
        );
        $result = $wpdb->get_var($query);
